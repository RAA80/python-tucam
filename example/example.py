...
with TUCAM() as cam:    # либо cam.TUCAM_Dev_Open() в начале и cam.TUCAM_Dev_Close() в конце
    ...
    print("TUCAM_Prop_SetValue (TUIDP_EXPOSURETM) = {}".format(cam.TUCAM_Prop_SetValue(attr_id=TUCAM_IDPROP.TUIDP_EXPOSURETM, value=10*prop_attr.dbValStep)))
    ...
    print("TUCAM_File_SaveImage = {}".format(cam.TUCAM_File_SaveImage(path="./screenshot")))

    # Write video stream

    # print("TUCAM_Rec_Start = {}".format(cam.TUCAM_Rec_Start(path="./video.avi")))
    # for _ in range(100):
    ...
    print("TUCAM_Reg_Read = {}".format(cam.TUCAM_Reg_Read(reg_id=TUREG_TYPE.TUREG_SN)))
    # print("TUCAM_Reg_Write = {}".format(cam.TUCAM_Reg_Write(reg_id=TUREG_TYPE.TUREG_SN, value=register)))


    roi = cam.TUCAM_Cap_GetROI()
    print("TUCAM_Cap_GetROI = {}".format(roi))
    print("    bEnable      = {}".format(roi.bEnable))
    print("    nHOffset     = {}".format(roi.nHOffset))
    print("    nVOffset     = {}".format(roi.nVOffset))
    print("    nWidth       = {}".format(roi.nWidth))
    print("    nHeight      = {}".format(roi.nHeight))

    roi_ = TUCAM_ROI_ATTR()
    roi_.bEnable = roi.bEnable
    roi_.nHOffset = roi.nHOffset
    roi_.nVOffset = roi.nVOffset
    roi_.nWidth = roi.nWidth
    roi_.nHeight = roi.nHeight
    print("TUCAM_Cap_SetROI = {}".format(cam.TUCAM_Cap_SetROI(roi_)))
